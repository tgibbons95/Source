  ...
  _height(0)
{
    ...
    GLUtils::loadShader("..\\Data\\fontVertex.shader", GL_VERTEX_SHADER, _programId);
    GLUtils::loadShader("..\\Data\\fontFragment.shader", GL_FRAGMENT_SHADER, _programId);
...
void FTLabel::setPixelSize(int size) {
    ...
    if(_fontAtlas.count(_pixelSize) == 0)
        _fontAtlas.insert(std::pair<int, std::shared_ptr<FontAtlas>>(_pixelSize, std::shared_ptr<FontAtlas>(new FontAtlas(_face, _pixelSize))));
